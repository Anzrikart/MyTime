...
const updateClasses = () => {
    ...
    const userEnrollments = database.enrollments.filter(e => e.userId === userId);
    const userClassIds = userEnrollments.map(e => e.classId);
    const availableClasses = database.classes.filter(c => userClassIds.includes(c.id) && c.schoolId === schoolId);

    // Sort classes by gradeLevel in descending order (highest/latest year first)
    availableClasses.sort((a, b) => b.gradeLevel - a.gradeLevel);
...
const convertTo12HourRange = rangeStr => {
    return rangeStr.split(' - ').map(t => convertTo12Hr(t.trim())).join(' - ');
};
...
const loadTimetable = () => {
    ...
    classInfo.innerHTML = `<strong>Class:</strong> ${
        selectedClass.name
        ...
        CONFIG.daysOfWeek.forEach(day => {
            ...
            if (dayData.code) {
                cell.className = `subject-cell ${
                    getSubjectColor(dayData.code)
                }`;

                // Get subject details from database
                const subject = database.getSubjectByCode(dayData.code);
                const subjectName = subject ? subject.name : dayData.code;
                ...
                cell.innerHTML = `
                    ...
                    ${
                    dayData.teacherCode ? `<span class="teacher-name">${
                        dayData.teacherCode
                    }</span>` : ''
                }
                `;
